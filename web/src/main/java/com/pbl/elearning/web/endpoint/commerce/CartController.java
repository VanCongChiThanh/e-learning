...
public class CartController {
    ...
    @ApiOperation(value = "Apply coupon", notes = "Apply a discount coupon to the cart")
    public ResponseEntity<ResponseDataAPI> applyCoupon(
            @Valid @RequestBody ApplyCouponRequest request,
            Authentication authentication) {

        Long userId = getUserIdFromAuthentication(authentication);
        CartResponse cartResponse = cartService.applyCoupon(request, userId);

        return ResponseEntity.ok(
                ResponseDataAPI.success(cartResponse, "Coupon applied successfully"));
    }

    @DeleteMapping("/coupon")
    @ApiOperation(value = "Remove coupon", notes = "Remove the applied coupon from the cart")
    public ResponseEntity<ResponseDataAPI> removeCoupon(Authentication authentication) {

        Long userId = getUserIdFromAuthentication(authentication);
        CartResponse cartResponse = cartService.removeCoupon(userId);

        return ResponseEntity.ok(
                ResponseDataAPI.success(cartResponse, "Coupon removed successfully"));
    }
    ...
    public ResponseEntity<ResponseDataAPI> getCartCourseIds(Authentication authentication) {
        ...
        Long userId = getUserIdFromAuthentication(authentication);
        List<Long> courseIds = cartService.getCartCourseIds(userId);
