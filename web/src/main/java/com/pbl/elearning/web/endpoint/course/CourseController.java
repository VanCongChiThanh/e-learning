...
public class CourseController {
        ...
        @GetMapping("/not-purchased/page")
        @PreAuthorize("hasAnyRole('LEARNER', 'ADMIN')")
        public ResponseEntity<ResponseDataAPI> getCoursesUserNotPurchasedYet(
                        ...
                        @Filter Specification<Course> specification) {
                Pageable pageable = PagingUtils.makePageRequest(sort, order, page, paging);
                Page<CourseResponeInstructor> coursesPage = courseService.courseUserNotPurchasesResponse(
                                user.getId(), pageable, specification);
                PageInfo pageInfo = new PageInfo(
                                pageable.getPageNumber() + 1,
                                coursesPage.getTotalPages(),
                                coursesPage.getTotalElements());
