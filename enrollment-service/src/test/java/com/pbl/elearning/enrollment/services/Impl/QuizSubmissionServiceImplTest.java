...
class QuizSubmissionServiceImplTest {
    ...
    // @Test
    // @DisplayName("Should calculate time taken correctly")
    // void shouldCalculateTimeTakenCorrectly() {
    //     // Given
    //     OffsetDateTime startTime = OffsetDateTime.now().minus(25, ChronoUnit.MINUTES);
    //     submissionRequest.setStartedAt(startTime);
    ...
    //     QuizSubmission savedSubmission = createMockSubmission();
    //     savedSubmission.setTimeTakenMinutes(25);
    //     when(quizSubmissionRepository.save(any(QuizSubmission.class))).thenReturn(savedSubmission);

    //     // When
    //     QuizSubmissionResponse result = quizSubmissionService.submitQuiz(submissionRequest, userId);

    //     // Then
    //     assertEquals(25, result.getTimeTakenMinutes());
        
    //     // Verify that submission was saved with correct time
    //     ArgumentCaptor<QuizSubmission> submissionCaptor = ArgumentCaptor.forClass(QuizSubmission.class);
    //     verify(quizSubmissionRepository).save(submissionCaptor.capture());
    //     QuizSubmission capturedSubmission = submissionCaptor.getValue();
    //     assertTrue(capturedSubmission.getTimeTakenMinutes() >= 24 && capturedSubmission.getTimeTakenMinutes() <= 26);
    // }
