...
//        when(payOSConfig.getDefaultExpirationMinutes()).thenReturn(30); // 30 phút
    ...
    void createPayment_ShouldCreateNewPayment_WhenNoPendingPaymentExists() throws Exception {
        ...
        // --- Act ---
        PaymentResponse response = paymentService.createPayment(createPaymentRequest, userId);
        ...
        // 2. Kiểm tra paymenRepository.save được gọi đúng 2 lần và với trạng thái đúng
    ...
    void createPayment_ShouldCreateNewPayment_WhenExistingPaymentIsExpired() throws Exception {
        ...
        expiredPayment.setExpiresAt(new Timestamp(System.currentTimeMillis() - 60000));
        ...
        assertEquals("http://payos.new.checkout.url", savedPayments.get(2).getCheckoutUrl());
    }
    ...
    void createPayment_ShouldThrowExceptionAndSaveFailedPayment_WhenPayOSApiFails() throws Exception {
        ...
        // 1. Kiểm tra nội dung của exception
        assertTrue(exception.getMessage().contains("Failed to create payment link: PayOS API Error"));
