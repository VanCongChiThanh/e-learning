...
// CreateOrderRequest.OrderItemRequest item1 = new
// CreateOrderRequest.OrderItemRequest();
// item1.setCourseId(courseId1);
// item1.setCoursePrice(100.0);

// CreateOrderRequest.OrderItemRequest item2 = new
// CreateOrderRequest.OrderItemRequest();
...
// when(orderRepository.findByUserIdAndCourseIdAndStatusIn(eq(userId),
// eq(courseId1), anyList()))
// .thenReturn(Optional.empty());
// when(orderRepository.findByUserIdAndCourseIdAndStatusIn(eq(userId),
// eq(courseId2), anyList()))
// .thenReturn(Optional.empty());
...
// CourseClient.CourseResponse courseResponse1 = new
// CourseClient.CourseResponse();
// courseResponse1.setData(Map.of("title", "Course 1 Title", "image",
// "img1.jpg"));
// when(courseClient.getCourseDetails(courseId1.toString())).thenReturn(courseResponse1);

// CourseClient.CourseResponse courseResponse2 = new
// CourseClient.CourseResponse();
// courseResponse2.setData(Map.of("title", "Course 2 Title", "image",
// "img2.jpg"));
...
// assertEquals(0, new
// BigDecimal("150.0").compareTo(savedOrder.getTotalAmount()));
...
// assertEquals(0, new
// BigDecimal("150.0").compareTo(response.getFinalAmount()));
...
// assertTrue(exception.getMessage().contains("User has already purchased
// course:"));
...
// assertEquals(2, response.getItems().size());
// assertEquals(0, new
// BigDecimal("150.0").compareTo(response.getFinalAmount())); // Tổng tiền lấy
// có lưu chính xác không
...
// assertEquals(2, savedOrder.getItems().size());
// assertEquals(0, new
// BigDecimal("150.0").compareTo(savedOrder.getTotalAmount())); // Tổng tiền
// được set chính xác không
...
// assertTrue(exception.getMessage().contains("You have already purchased
// course: " + courseId1));
